...
pub fn part1() {
    let i: Vec<HashMap<String, String>> = input();
    let expected_fields: Vec<&str> = vec!["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];
    let optional_fields: Vec<&str> = vec!["cid"];
    let valid_count = i
        .iter()
        .filter(|passport| valid_passport(passport, &expected_fields))
        .count();
    println!("{:?}", valid_count);
}
...
pub fn part2() {
    ...
    let valid_count = i
        .iter()
        .filter(|passport| valid_passport(passport, &expected_fields))
        .filter(|passport| passport.keys().all(|k: &String| valid_field(k, passport.get(k).unwrap())))
        .count();
    println!("{:?}", valid_count);
}

fn valid_field(arg: &String, val: &String) -> bool {
    println!("Sprawdzam {} {}", arg, val);
    let r = match arg.as_ref() {
        "byr" => valid_int(val, 1920, 2002),
        "iyr" => valid_int(val, 2010, 2020),
        "eyr" => valid_int(val, 2020, 2030),
        "hgt" => valid_hgt(val),
        "hcl" => valid_hcl(val),
        "ecl" => valid_ecl(val),
        "pid" => valid_pid(val),
...
fn valid_pid(arg: &String) -> bool {
    arg.len() == 9 &&
        arg.chars().all(|c| match c.to_digit(10) {
            None => false,
            Some(_) => true
        })
}

fn valid_ecl(arg: &String) -> bool {
    match arg.as_ref() {
        "amb" => true,
        "blu" => true,
        "brn" => true,
...
fn valid_hcl(arg: &String) -> bool {
    let first = arg.chars().next().unwrap();
    let rest: String = arg.chars().skip(1).collect();
    first == '#' && (rest.chars().all(|c| c == '1' || c == '2' || c == '3' || c == '4' || c == '5' || c == '6' || c == '7' || c == '8' || c == '9' || c == '0' || c == 'a' || c == 'b' || c == 'c' || c == 'd' || c == 'e' || c == 'f'))
}

fn valid_hgt(arg: &String) -> bool {
    let c: Vec<char> = arg.chars().collect();
    let value: String = c[0..c.len() - 2].iter().collect();
    let unit: String = c[c.len() - 2..c.len()].iter().collect();
    match unit.as_ref() {
        "cm" => valid_int(&value, 150, 193),
        "in" => valid_int(&value, 59, 76),
        _ => false
    }
}

fn valid_int(arg: &String, y_min: u32, y_max: u32) -> bool {
    let byr_int: u32 = arg.parse().unwrap();
    byr_int >= y_min && byr_int <= y_max
}

fn valid_passport(pass: &HashMap<String, String>, expected_fields: &Vec<&str>) -> bool {
    expected_fields
        .iter()
        .all(|&field| pass.contains_key(field))
}

fn to_tuple(arg: &str, denominator: char) -> (String, String) {
    let v: Vec<String> = arg.split(denominator).map(|x| x.to_string()).collect();
    return (v[0].clone(), v[1].clone());
}

fn input() -> Vec<HashMap<String, String>> {
//     let i = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
// iyr:2011 ecl:brn hgt:59in";
    let i = "ecl:#eef340 eyr:2023 hcl:#c0946f pid:244684338 iyr:2020 cid:57 byr:1969 hgt:152cm
...
pid:274974402 hgt:183cm";
    let double_space = "  ";
    let r: Vec<HashMap<String, String>> = i
        .replace('\n', " ")
        .split(double_space)
        .map(|s| s.split(' ').map(|s| to_tuple(s, ':')).collect())
        .collect();
    r
}
