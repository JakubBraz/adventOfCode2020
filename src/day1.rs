pub fn part1() {
    ...
    let inputs: Vec<i32> = input().to_vec();
    let (v1, v2) = find_sum(inputs.clone());
    println!("{} {}", v1, v2);
    println!("{}", v1 * v2);
    println!("{}", find_mult_sum(combine(2, to_vec_of_vec(&inputs), &inputs), 2020));
    println!("{}", find_mult_sum(combine(3, to_vec_of_vec(&inputs), &inputs), 2020));
}

fn find_mult_sum(arg: Vec<Vec<i32>>, target: i32) -> i32 {
    for nums in arg {
        if target == nums.iter().sum() {
...
fn to_vec_of_vec(arr: &Vec<i32>) -> Vec<Vec<i32>> {
    arr.iter().map(|&x| vec![x]).collect()
}

fn combine(size: i32, vecs: Vec<Vec<i32>>, arr: &Vec<i32>) -> Vec<Vec<i32>> {
    if size == 1 {
        return vecs;
    }
    ...
    for v in vecs.iter() {
        let temp: Vec<Vec<i32>> = arr.iter().map(|&x| -> Vec<i32> {
            let mut t = v.clone();
            t.push(x);
            t
        }).collect();
    ...
    combine(size - 1, res, arr)
}

fn find_sum(inputs: Vec<i32>) -> (i32, i32) {
