pub fn part1() {
    let i: Vec<(i32, i32, char, String)> = inputs();
    let size = i.into_iter().filter(|(v_min, v_max, letter, val)| -> bool {
        let filtered: i32 = val.chars().filter(|c| c == letter).count() as i32;
        filtered >= *v_min && filtered <= *v_max
    }).count();
    println!("{}", size);
}
...
pub fn part2() {
    let i: Vec<(i32, i32, char, String)> = inputs();
    let size = i.into_iter().filter(|(v_min, v_max, letter, val)| -> bool {
        let first = val.chars().nth(*v_min as usize - 1).unwrap();
        let second = val.chars().nth(*v_max as usize - 1).unwrap();
        first != second && (first == *letter || second == *letter)
    }).count();
    println!("{}", size);
}

fn inputs() -> Vec<(i32, i32, char, String)> {
    let i = "6-10 p: ctpppjmdpppppp
...
3-4 v: vvxxv
8-11 t: tttttttcttm";
    let ss = i.replace(":", "").replace("-", " ");
    let s: Vec<(i32, i32, char, String)> = ss.split("\n")
        .map(|x| x.split(" ").collect())
        .map(|x: Vec<&str>| -> (i32, i32, char, String) {
            match &x[..] {
                &[e1, e2, e3, e4] => (e1.to_string().parse().unwrap(), e2.to_string().parse().unwrap(), e3.chars().next().unwrap(), e4.to_string()),
                _ => unreachable!()
            }
        })
        .collect();
