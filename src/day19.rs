...
pub fn part1() {
    let (rules, messages) = parse_to_struct();
    let mut r = build_regex("0", &rules);
    r = format!("^{}$", r);
    let r = Regex::new(&r).unwrap();
    let r = messages.iter().filter(|m| r.is_match(m)).count();
    println!("{}", r);
}

pub fn part2() {
    let (mut rules, messages) = parse_to_struct();
    rules.insert("8".to_string(), " 42 x ".to_string());
    rules.insert("11".to_string(), " 42 y 31 ".to_string());
    let mut r = build_regex("0", &rules);
    r = format!("^{}$", r);
    let for_x = build_regex("42", &rules);
    let for_y = build_regex("31", &rules);

    r = r.replace("x", &format!("({})*", for_x));
    let to_regexes: Vec<String> = (0..5).into_iter().map(|i| { // this should be solved better, but let it be for now
        if i > 0 {
            r = r.replace("y", &format!("(({})y({}))", for_x, for_y));
        }
        r.clone()
    }).collect();
    let to_regexes = to_regexes.iter().map(|re| re.replace("y", ""));
    let regexes: Vec<Regex> = to_regexes.map(|re| Regex::new(&re).unwrap()).collect();
    let result = messages.iter().filter(|&m| regexes.iter().any(|reg| reg.is_match(&m))).count();
    println!("{}", result);
}

fn build_regex(ind: &str, rules: &HashMap<String, String>) -> String {
    let mut result: String = rules.get(ind).unwrap().clone();
    while result.chars().any(|c| c.is_digit(10)) {
        let num: &str = result.split(' ')
            .skip_while(|&s| s.parse::<u32>().is_err())
            .next()
            .unwrap();
        result = result.replace(&format!(" {} ", num), &format!(" ( {} ) ", rules.get(num).unwrap()));
    }
    result.replace(' ', "")
}

fn parse_to_struct() -> (HashMap<String, String>, Vec<String>) {
    let (rules, messages) = parse_input(&input());
    let messages: Vec<String> = messages.split('\n').map(|x| x.to_string()).collect();
    let rules: HashMap<String, String> = rules.replace('|', " | ").split('\n').map(|line| {
        let splitted: Vec<&str> = line.split(":").collect();
        (splitted[0].to_string(), format!(" {} ", splitted[1].to_string()))
    }).collect();
    (rules.clone(), messages)
}

fn parse_input(arg: &str) -> (String, String) {
    let s: Vec<String> = arg.split("\n\n").map(|s| s.chars().collect()).collect();
    (s[0].replace(": ", ":").replace(" | ", "|").clone(), s[1].clone())
}
...
baaababaaabaababbaabaabb".to_string()
//     "42: 9 14 | 10 1
// 9: 14 27 | 1 26
// 10: 23 14 | 28 1
// 1: a
