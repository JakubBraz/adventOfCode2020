...
pub fn part1() {
    ...
    let my_bag: String = "shinygold".to_string();
    let shinygold_reachable = values.iter()
        .map(|(color, set)| reachable(color, my_bag.as_str(), &values))
        .filter(|b| *b)
        .count();
    println!("{:?}", shinygold_reachable);
}

pub fn part2() {
    let values: HashMap<String, HashSet<(u32, String)>> = parse_input();
    let my_bag: String = "shinygold".to_string();
    println!("{}", count_bags(my_bag.as_str(), &values));
}

fn count_bags(current: &str, map: &HashMap<String, HashSet<(u32, String)>>) -> u32 {
    if map.get(current).unwrap().is_empty() {
        return 0;
    }

    return map.get(current).unwrap().iter().map(|(c, v)| c + c * count_bags(v, map)).sum();
}

fn reachable(current: &str, target: &str, map: &HashMap<String, HashSet<(u32, String)>>) -> bool {
    if map.get(current).unwrap().is_empty() {
        return false;
    }

    let string_map: HashSet<String> = map.get(current).unwrap().iter().map(|(c, v)| v.clone()).collect();
    if string_map.contains(target) {
        return true;
    }

    map.get(current).unwrap().iter().map(|(c, e)| reachable(e, target, map)).fold(false, |e1, e2| e1 || e2)
}

fn parse_input() -> HashMap<String, HashSet<(u32, String)>> {
    let s: String = input();
    let parsed: Vec<(&str, &str)> = s
        .split('\n')
        .map(|line| -> (&str, &str) {
        ...
        .collect();
    let parsed: Vec<(&str, Vec<&str>)> = parsed.iter()
        .map(|(e1, e2)| (e1.strip_suffix(' ').unwrap(), e2.split(',').collect()))
        .collect();
    let parsed: HashMap<String, Vec<&str>> = parsed.iter()
        .map(|(k, v)| (k.to_string(), v.clone()))
        .collect();
    let parsed: HashMap<String, Vec<(u32, String)>> = parsed.iter()
        .map(|(e1, e2)| (e1.replace(' ', ""), e2.iter().map(|s| -> String {
            s
                .replace(" bags", "")
                .replace("bag", "")
            ...
            .collect()))
        .collect();
    let parsed: HashMap<String, HashSet<(u32, String)>> = parsed.iter()
        .map(|(k, v)| (k.clone(), v.iter().filter(|&(c, v)| v.ne("noother")).cloned().collect()))
        .collect();
    parsed
}

fn split_number(arg: &str) -> (u32, String) {
    if arg.eq("noother") {
        return (0, arg.to_string());
    }
    let x_str: String = arg.chars().take_while(|&c| c.is_numeric()).collect();
    let x: u32 = x_str.parse().unwrap();
    (x, arg.strip_prefix(&x_str).unwrap().to_string())
}
...
// dotted black bags contain no other bags.".to_string();
    return "bright gray bags contain 2 bright gold bags, 5 dull lavender bags.
...
plaid blue bags contain 1 dull indigo bag, 4 wavy black bags, 4 clear red bags.".to_string();
}
