...
pub fn part1() {
    let st: String = input();
    // let set_list: Vec<HashSet<char>> = st
    let sum: usize = st
        .split("\n\n")
        .map(|group| -> HashSet<char> { group.replace('\n', "").chars().collect() })
        .map(|hs| hs.len())
        .sum();
    println!("{}", sum);

    let ss1: HashSet<char> = "aabc".chars().collect();
    let ss2: HashSet<char> = "bbcc".chars().collect();
    let r: HashSet<char> = ss1.intersection(&ss2).cloned().collect();
    println!("{:?}", r);
    let sum2: usize = st
        // let sum2: HashSet<char> = st
        .split("\n\n")
        .map(|group| -> HashSet<char> { group_intersection(group) })
        .map(|i| i.len())
        .sum();
    println!("{}", sum2);
}

fn group_intersection(g: &str) -> HashSet<char> {
    // let sets: Vec<HashSet<char>> = g
    let sets: HashSet<char> = g
        .split('\n')
        .map(|line| -> HashSet<char> { line.chars().collect() })
        .fold(HashSet::from(('a'..='z').collect()), |set1, set2| set1.intersection(&set2).cloned().collect());
    sets
}
